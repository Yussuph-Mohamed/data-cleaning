...
compute_corr <- function(attr_name, dataset, target_name){
        if(is.numeric(dataset[[attr_name]])){
                pvalue <- oneway.test(as.formula(paste(attr_name, target_name, sep="~")), data=dataset)$p.value
        } else { #it is factor
...
```{r model1}
m1 <- glm(Survived ~ Pclass + Sex + Parch + Fare + Embarked + Title, family=binomial(link='logit'), data=train)
summary(m1)
```
...
3. R Documentation. Shapiro-Wilk Normality Test. http://finzi.psych.upenn.edu/R/library/stats/html/shapiro.test.html. 
4. R Documentation. Levene's Test. http://finzi.psych.upenn.edu/R/library/car/html/leveneTest.html
5. R Documentation. Test for Equal Means in a One-Way Layout. http://finzi.psych.upenn.edu/R/library/stats/html/oneway.test.html
6. R Documentation. Pearson's Chi-squared Test for Count Data.
