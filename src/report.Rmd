...
kable(sapply(all_clean,count_empty),format="pandoc",col.names=c("Empty"),digits=4)
```
...
kable(rbind(shapiro.wilk,levene),format="pandoc",digits=4)
```
...
kable(sapply(names(train[-ncol(train)]),compute_corr,dataset=train,
             target_name="Survived"), format="pandoc",
      col.names=c("Correlation"), digits=4)
```
